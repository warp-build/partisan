...
%% -----------------------------------------------------------------------------
%% @doc This module is an adaptation of Erlang's `proc_lib' module which
...
-spec spawn(Node, Module, Function, Args) -> pid2() when
      Node :: node(),
      Module :: module(),
      Function :: atom(),
      Args :: [term()].
...
-spec spawn_link(Node, Fun) -> pid2() when
      Node :: node(),
      Fun :: function().
...
-spec spawn_link(Node, Module, Function, Args) -> pid2() when
      Node :: node(),
      Module :: module(),
      Function :: atom(),
      Args :: [term()].
...
-spec spawn_opt(Fun, SpawnOpts) -> pid2() | {pid(), reference()} when
      Fun :: function(),
...
-spec spawn_opt(Node, Function, SpawnOpts) -> pid2() | {pid2(), reference2()} when
      Node :: node(),
      Function :: function(),
      SpawnOpts :: [spawn_option()].
...
crash_report(Class, Reason, StartF, Stacktrace) ->
    ?LOG_ERROR(#{label=>{proc_lib,crash},
                 report=>[my_info(Class, Reason, StartF, Stacktrace),
                          linked_info(self())]},
               #{domain=>[otp,sasl],
                 report_cb=>fun partisan_proc_lib:report_cb/2,
...
get_process_messages(Pid) ->
    ...
    case not partisan:is_self(Pid) orelse Depth =:= unlimited of
        true ->
