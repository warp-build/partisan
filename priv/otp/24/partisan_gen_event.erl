...
start() ->
    partisan_gen:start(?MODULE, nolink, ?NO_CALLBACK, [], []).

-spec start(emgr_name() | [option()]) -> start_ret().
start(Name) when is_tuple(Name) ->
    partisan_gen:start(?MODULE, nolink, Name, ?NO_CALLBACK, [], []);
start(Options) when is_list(Options) ->
    partisan_gen:start(?MODULE, nolink, ?NO_CALLBACK, [], Options).

-spec start(emgr_name(), [option()]) -> start_ret().
start(Name, Options) ->
    partisan_gen:start(?MODULE, nolink, Name, ?NO_CALLBACK, [], Options).

-spec start_link() -> start_ret().
start_link() ->
    partisan_gen:start(?MODULE, link, ?NO_CALLBACK, [], []).

-spec start_link(emgr_name() | [option()]) -> start_ret().
start_link(Name) when is_tuple(Name) ->
    partisan_gen:start(?MODULE, link, Name, ?NO_CALLBACK, [], []);
start_link(Options) when is_list(Options) ->
    partisan_gen:start(?MODULE, link, ?NO_CALLBACK, [], Options).

-spec start_link(emgr_name(), [option()]) -> start_ret().
start_link(Name, Options) ->
    partisan_gen:start(?MODULE, link, Name, ?NO_CALLBACK, [], Options).

-spec start_monitor() -> start_mon_ret().
start_monitor() ->
    partisan_gen:start(?MODULE, monitor, ?NO_CALLBACK, [], []).

-spec start_monitor(emgr_name() | [option()]) -> start_mon_ret().
start_monitor(Name) when is_tuple(Name) ->
    partisan_gen:start(?MODULE, monitor, Name, ?NO_CALLBACK, [], []);
start_monitor(Options) when is_list(Options) ->
    partisan_gen:start(?MODULE, monitor, ?NO_CALLBACK, [], Options).

-spec start_monitor(emgr_name(), [option()]) -> start_mon_ret().
start_monitor(Name, Options) ->
    partisan_gen:start(?MODULE, monitor, Name, ?NO_CALLBACK, [], Options).
...
receive_response(RequestId, Timeout) ->
    case partisan_gen:receive_response(RequestId, Timeout) of
        {reply, {error, _} = Err} -> Err;
        Return -> Return
    end.
...
check_response(Msg, RequestId) ->
    case partisan_gen:check_response(Msg, RequestId)  of
        {reply, {error, _} = Err} -> Err;
        Return -> Return
    end.
...
stop(M, Reason, Timeout) ->
    partisan_gen:stop(M, Reason, Timeout).

rpc(M, Cmd) ->
    {ok, Reply} = partisan_gen:call(M, self(), Cmd, infinity),
    Reply.

call1(M, Handler, Query) ->
    Cmd = {call, Handler, Query},
    try partisan_gen:call(M, self(), Cmd) of
    {ok, Res} ->
        Res
    catch
    exit:Reason ->
...
call1(M, Handler, Query, Timeout) ->
    Cmd = {call, Handler, Query},
    try partisan_gen:call(M, self(), Cmd, Timeout) of
    {ok, Res} ->
        Res
    catch
    exit:Reason ->
...
loop(Parent, ServerName, MSL, HibernateAfterTimeout, Debug, true) ->
     partisan_proc_lib:hibernate(?MODULE, wake_hib, [Parent, ServerName, MSL, HibernateAfterTimeout, Debug]);
...
format_status(Opt, StatusData) ->
    ...
    Header = partisan_gen:format_status_header("Status for event handler",
                                      ServerName),
